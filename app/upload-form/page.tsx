...
const UploadForm = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await ffmpeg.run('-i', 'input.mp4', '-i', 'input.mp3', '-c', 'copy', '-r', '20', '-b:a', '320K', 'output.mp4');

      const data = ffmpeg.FS('readFile', 'output.mp4');
      const url = URL.createObjectURL(new Blob([data.buffer], { type: 'video/mp4' }));

      // Display merged video preview
      if (videoRef.current) {
        videoRef.current.src = url;
      }
    } catch (error) {
      console.error('Error converting text to speech or merging audio/video:', error);
    } finally {
      setIsLoading(false); // Set loading to false after the upload process is complete
    }
  };
